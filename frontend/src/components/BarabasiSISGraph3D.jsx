...
function BarabasiSISGraph3D({ data, nodesWithCentrality, onNodeInfo, highlightedLinks = [], highlightId, onResetView, beta, gamma, selectedUser}) {
  const fgRef = useRef();
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('baSISPropagationStart'));
    ...
    const animateLink = (highlight, index) => {
      ...
      const animationEndTimeout = setTimeout(() => {
        if (linkObj) {
          ...
          const propagationEvent = new CustomEvent('baSISPropagationUpdate', {
            detail: {
          ...
          if (allPropagated && Math.random() < gamma) {
            nodeStates[targetId] = 'susceptible';
            // Disparar evento para la recuperación del nodo target a susceptible
            const recoveryEvent = new CustomEvent('baSISPropagationUpdate', {
              detail: {
                t: highlight.timeStep,
                sender: targetId,
                receiver: targetId, // El mismo nodo se recupera a susceptible
    ...
    const finalTimeout = setTimeout(() => {
      ...
      const finishedEvent = new CustomEvent('baSISPropagationFinished');
      ...
      <ForceGraph3D
        ...
        nodeThreeObject={node => {
          ...
          const material = new THREE.MeshBasicMaterial({
            color: nodeState === 'infected' ? infectedNodeColor : susceptibleNodeColor,
            transparent: true,
            opacity: 0.8,
          });
...
export default BarabasiSISGraph3D;