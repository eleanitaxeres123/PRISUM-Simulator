...
function HolmeKimBehaviorGraph3D({ data, nodesWithCentrality, onNodeInfo, highlightId, highlightedLinks = [], onResetView, propagationLog = [], currentViewMode }) {
  const fgRef = useRef();
  ...
  const filteredData = useMemo(() => {
    ...
    const linksCopy = data.links.map(link => ({
      ...link,
      source: nodesCopy.find(n => n.id === (link.source.id || link.source)) || link.source,
      target: nodesCopy.find(n => n.id === (link.target.id || link.target)) || link.target,
    }));
  ...
  // Inicializar posiciones originales y centrar la red
  useEffect(() => {
    if (!isTransitioning.current && fgRef.current && filteredData.nodes.length > 0) {
  ...
  }, [filteredData.nodes, isExtensivePropagation]);
  ...
  const showLegend = currentViewMode === 'holme-kim-behavior';
