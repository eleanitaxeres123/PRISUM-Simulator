...
function HolmeKimSIRGraph3D({ data, nodesWithCentrality, onNodeInfo, highlightedLinks = [], highlightId, onResetView, beta, gamma, selectedUser }) {
  const fgRef = useRef();
...
const filteredData = useMemo(() => {
  ...
  useEffect(() => {
    ...
    // Dispatch start event (para Holme-Kim)
    window.dispatchEvent(new CustomEvent('hkSIRPropagationStart'));
    ...
    const animateLink = (highlight, index) => {
      ...
      const animationEndTimeout = setTimeout(() => {
        if (linkObj) {
          ...
          // Disparar evento de propagación (para Holme-Kim)
          const propagationEvent = new CustomEvent('hkSIRPropagationUpdate', {
            detail: {
          ...
          if (allPropagated && Math.random() < gamma) {
            ...
            // Disparar evento para la recuperación del nodo target (Holme-Kim)
            const recoveryEvent = new CustomEvent('hkSIRPropagationUpdate', {
              detail: {
                t: highlight.timeStep,
                sender: targetId,
                receiver: targetId,
    ...
    const finalTimeout = setTimeout(() => {
      // Disparar evento de finalización de propagación (Holme-Kim)
      window.dispatchEvent(new CustomEvent('hkSIRPropagationFinished'));
