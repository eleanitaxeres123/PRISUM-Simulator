...
function HolmeKimSISGraph3D({ data, nodesWithCentrality, onNodeInfo, highlightedLinks = [], highlightId, onResetView, beta, gamma, selectedUser }) {
  const fgRef = useRef();
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('hkSISPropagationStart'));
    ...
    const animateLink = (highlight, index) => {
      ...
      const animationEndTimeout = setTimeout(() => {
        if (linkObj) {
          ...
          const propagationEvent = new CustomEvent('hkSISPropagationUpdate', {
            detail: {
          ...
          if (allPropagated && Math.random() < gamma) {
            ...
            const recoveryEvent = new CustomEvent('hkSISPropagationUpdate', {
              detail: {
                t: highlight.timeStep,
                sender: targetId,
                receiver: targetId,
    ...
    const finalTimeout = setTimeout(() => {
      // Disparamos el evento de finalización de propagación
      const finishedEvent = new CustomEvent('hkSISPropagationFinished');
...
export default HolmeKimSISGraph3D;