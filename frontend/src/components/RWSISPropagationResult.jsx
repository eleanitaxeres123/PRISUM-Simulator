...
export default function RWSISPropagationResult({ selectedUser, onClose }) {
  const [logEntries, setLogEntries] = useState([]);
  const [currentIndex, setCurrentIndex] = useState(-1); // Start with -1 to show nothing until first event
  const [isPropagationFinished, setIsPropagationFinished] = useState(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener('rwSISPropagationStart', handlePropagationStart);
    window.addEventListener('rwSISPropagationUpdate', handlePropagationUpdate);
    window.addEventListener('rwSISPropagationFinished', handlePropagationFinished);

    return () => {
      window.removeEventListener('rwSISPropagationStart', handlePropagationStart);
      window.removeEventListener('rwSISPropagationUpdate', handlePropagationUpdate);
      window.removeEventListener('rwSISPropagationFinished', handlePropagationFinished);
    };
  }, []);
  ...
  if (logEntries.length === 0 || currentIndex < 0) {
    return (
      <div className="ba-sis-propagation-result">
        <div className="propagation-result-content" ref={logTableRef}>
          <div className="header">
            <h2 className="title">Propagación SIS</h2>
          </div>
        ...
        <div className="log-table">
          ...
          {logEntries.map((entry, index) => (
            ...
            >
              ...
              <span>{entry.state === 'infected' ? 'Infectado' : 'Susceptible'}</span>
            </div>
          ))}
        </div>
        {isPropagationFinished && (
...
RWSISPropagationResult.propTypes = {
