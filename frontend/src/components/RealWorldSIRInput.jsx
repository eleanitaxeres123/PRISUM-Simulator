...
import './RealWorldSIRInput.css';
import './PolicySeedingInput.css';

export default function RealWorldSIRInput({ nodes, onStartPropagation }) {
  ...
  return (
    <div className="sir-navbar">
      ...
      <div className="sir-input-container">
                <label htmlFor="rw-sir-propagation-name" className="sir-label">Nombre de Propagación</label>
                <input
                  id="rw-sir-propagation-name"
                  type="text"
                  value={propagationName}
                  onChange={(e) => setPropagationName(e.target.value)}
                  className="sir-input"
                  placeholder="Ej. Propagación SIR HR"
                  maxLength="100"
                />
              </div>
