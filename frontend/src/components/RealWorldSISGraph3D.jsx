...
function RealWorldSISGraph3D({ data, nodesWithCentrality, onNodeInfo, highlightedLinks = [], highlightId, onResetView, beta, gamma, selectedUser }) {
  const fgRef = useRef();
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('rwSISPropagationStart'));
    ...
    const animateLink = (highlight, index) => {
      ...
      const animationEndTimeout = setTimeout(() => {
        if (linkObj) {
          ...
          const propagationEvent = new CustomEvent('rwSISPropagationUpdate', {
            detail: {
          ...
          if (allPropagated && Math.random() < gamma) {
            ...
            const recoveryEvent = new CustomEvent('rwSISPropagationUpdate', {
              detail: {
                t: highlight.timeStep,
                sender: targetId,
                receiver: targetId,
    ...
    const finalTimeout = setTimeout(() => {
      ...
      const finishedEvent = new CustomEvent('rwSISPropagationFinished');
...
export default RealWorldSISGraph3D;