 ...
 * @param {Function} params.setBaSISPropagationStatus - Función para actualizar el estado de propagación
 * @param {Function} params.setBaSISBeta - Función para actualizar beta
 * @param {Function} params.setBaSISGamma - Función para actualizar gamma
 * @param {Function} params.setSelectedUser - Función para actualizar el usuario seleccionado
 * @param {Function} params.setShowBaSISPropagationResult - Función para mostrar/ocultar el resultado
 * @param {Function} params.setBaSISPropagationLog - Función para actualizar el log de propagación
 * @param {Function} params.setBaSISHighlightedLinks - Función para actualizar los enlaces destacados
 ...
 */
export const useSISPropagation = ({
  baGraphData,
   ...
   */
  const handleBaSISPropagation = useCallback(async ({ beta, gamma, k, policy, selectedUser, selectedUsers, propagationName }) => {
    ...
    if (!baGraphData.nodes.length) {
      setBaSISPropagationStatus('Por favor, genere una red Barabási-Albert primero.');
      return;
    }

    if (!user) {
      setBaSISPropagationStatus('Por favor, seleccione al menos un nodo inicial.');
      return;
    }

    // Configuración inicial
    setBaSISPropagationStatus('Iniciando propagación SIS…');
    setBaSISBeta(beta);
    setBaSISGamma(gamma);
    setSelectedUser(user); // Mantener compatibilidad con el estado existente
    setShowBaSISPropagationResult(true);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/propagate-ba-sis', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Error en la propagación SIS');
      }
      ...
      setBaSISPropagationLog(propagationLog);
      setBaSISHighlightedLinks(highlightedLinks);
      setHighlightId(user);
      setBaSISPropagationStatus('Propagación SIS completada.');
      
    } catch (error) {
      console.error('❌ Error en propagación SIS:', error);
      setBaSISPropagationStatus(`Error: ${error.message}`);
    }
  }, [
    baGraphData,
    setBaSISPropagationStatus,
  ...
  return handleBaSISPropagation;
};

export default useSISPropagation;
